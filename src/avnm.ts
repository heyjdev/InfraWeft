...
const ownershipPrefix = (plan: AvnmPlan) => `InfraWeft owner ${stableHash(JSON.stringify({
...
function terraform(plan: AvnmPlan) {
  ...
  return `# Azure Virtual Network Manager connectivity deployment generated by InfraWeft.
...
// Azure Virtual Network Manager configuration generated by InfraWeft.
...
# Azure Virtual Network Manager connectivity deployment generated by InfraWeft.
...
if [[ "$ACTIVE_CONNECTIVITY_COUNT" != "$OWNED_ACTIVE_CONNECTIVITY_COUNT" ]]; then
  echo 'Refusing complete goal-state commit: at least one target region has an active connectivity configuration not owned by InfraWeft.' >&2
  exit 1
fi
