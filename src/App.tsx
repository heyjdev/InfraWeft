...
const propertyTargetId = (path: string) => `property-target-${path.replace(/[^A-Za-z0-9_-]+/g, '-')}`
...
function Studio() {
  ...
  const [propertyIssueTarget, setPropertyIssueTarget] = useState<{ nodeId: string; field: string; requestId: number } | null>(null)
  ...
  useEffect(() => {
    if (!propertyIssueTarget || selectedId !== propertyIssueTarget.nodeId) return
    const frame = requestAnimationFrame(() => {
      const requested = document.getElementById(propertyTargetId(propertyIssueTarget.field))
      const target = requested ?? document.getElementById(propertyTargetId('overview'))
      if (!target) return
      const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
      target.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'center', inline: 'nearest' })
      if (!reduceMotion) target.animate([
        { outline: '3px solid rgba(0, 120, 212, .75)', outlineOffset: '5px', backgroundColor: 'rgba(0, 120, 212, .12)' },
        { outline: '3px solid rgba(0, 120, 212, 0)', outlineOffset: '5px', backgroundColor: 'rgba(0, 120, 212, 0)' },
      ], { duration: 1800, easing: 'ease-out' })
      const control = target.matches('input, select, textarea, button') ? target : target.querySelector<HTMLElement>('input:not(:disabled), select:not(:disabled), textarea:not(:disabled), button:not(:disabled)')
      control?.focus({ preventScroll: true })
    })
    return () => cancelAnimationFrame(frame)
  }, [propertyIssueTarget, selectedId])

  const selectTrace = (nodeId: string, field: string | null = null) => { setSelectedId(nodeId); setSelectedField(field) }
  const focusExportIssue = (nodeId: string, field?: string) => {
    const targetField = field ?? 'overview'
    setSelectedId(nodeId)
    setSelectedField(field ?? null)
    setPropertyIssueTarget({ nodeId, field: targetField, requestId: Date.now() })
    setNotice(field ? 'Property selected for export issue' : 'Resource selected for export issue')
  }
  ...
  function renderField(field: ResourceField, values: Record<string, unknown> = selected?.data as Record<string, unknown>, setValues: (next: Record<string, unknown>) => void = (next) => updateSelected(next), path = String(field.key)): React.ReactNode {
    ...
    if (field.type === 'block') {
      ...
      return <div id={propertyTargetId(path)} className="block-editor" key={path} onFocusCapture={() => focusField(path)}>
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <aside className="inspector-panel">
        ...
        {selected ? <div className="properties"><section id={propertyTargetId('overview')} className="property-overview"><div className="resource-heading"><span className="palette-icon" style={{ color: colors[selected.data.kind as ResourceKind], background: `${colors[selected.data.kind as ResourceKind]}15` }}>{(() => { const Icon = iconMap[selected.data.kind as ResourceKind]; return <Icon size={22}/> })()}</span><div><strong>{selected.data.label as string}</strong><small>{RESOURCE_LABELS[selected.data.kind as ResourceKind]}</small></div></div>
          ...
          <label id={propertyTargetId('label')} onFocus={() => focusField('label')}>Name<input value={selected.data.label as string} onChange={(e) => updateSelected({ label: e.target.value })}/></label>
          ...
          {selected.data.kind === 'frontDoor' ? <label id={propertyTargetId('region')} onFocus={() => focusField('region')}>Scope<input value="global" disabled/><small>AzureRM Front Door profiles do not have a location argument.</small></label> : selected.data.kind === 'subnet' ? <label id={propertyTargetId('region')} onFocus={() => focusField('region')}>Region (inherited)<input value={String(selectedParent?.data.region || 'Select a parent VNet')} disabled/></label> : <label id={propertyTargetId('region')} onFocus={() => focusField('region')}>Region<select value={(selected.data.region as string) || 'eastus'} onChange={(e) => updateSelected({ region: e.target.value })}>{AZURE_REGIONS.map((region) => <option key={region}>{region}</option>)}</select></label>}
          {selected.data.kind === 'subnet' ? <label id={propertyTargetId('resourceGroup')} onFocus={() => focusField('resourceGroup')}>Resource group (inherited)<input value={String(selectedParent?.data.resourceGroup || 'Select a parent VNet')} disabled/></label> : <label id={propertyTargetId('resourceGroup')} onFocus={() => focusField('resourceGroup')}>Resource group<input value={(selected.data.resourceGroup as string) || ''} onChange={(e) => updateSelected({ resourceGroup: e.target.value })}/></label>}</div></section>
